...
const BASE_URL = "https://www.vrbo.com/serp/g";

const apiRequestBuilder = (address, pageSize) => ({
...
const main = async () => {
  ...
  const inputPageSize = document.getElementById("pageSize").value;

  if (inputAddress === "" || inputPageSize === "")
    return alert("Please fill in all fields");

  const apiRequestBody = apiRequestBuilder(inputAddress, inputPageSize);
  const listings = await fetchListings(apiRequestBody);
  const extractedListings = extractParams(listings, [
    "listingId",
    "propertyMetadata",
    "rateSummary",
  ]);
  const transformedListings = transformData(extractedListings);
...
downloadButton.addEventListener("click", main, false);
